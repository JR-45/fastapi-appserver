...
Python-Packages_ (hier: mitglied).
...
`src\mitglied\router`. _import_ eines Moduls entspricht quasi dem Importieren der Datei
...
und  eine "Source-Distribution" z.B. `mitglied-<VERSION>.tar.gz`.

Das Wheel `mitglied-<VERSION>-py3-none-any.whl` wird dabei gemäß
https://packaging.python.org/en/latest/specifications/binary-distribution-format
erstellt. Die einzelnen Teile des Dateinamens bedeuten:

- Package-Name (hier: mitglied)
- Versionsnummer
...
- Daphne
  ...
  - Nur in der Kommandozeile nutzbar und deshalb nicht programmatisch integrierbar
    mit _FastAPI_, um einfach `python -m mitglied` aufrufen zu können
...
> Vor dem 1. Start von PostgreSQL muss man die Skripte `create-db-mitglied.sql`
> und `create-schema-mitglied.sql` aus dem Verzeichnis `extras\postgres\sql` nach
...
> nach `C:\Zimmermann\volumes\postgres\csv\mitglied`.
...
Durch das Kommando `uv run mitglied` kann man den Server starten: siehe `project.scripts`
in `pyproject.toml`.

Defaultmäßig wird beim Serverstart _uvicorn_ als ASGI-Server verwendet.
Stattdessen kann auch _hypercorn_ verwendet werden, indem man in `mitglied.toml`
bei `[mitglied.asgi]` das Tag `server` auf den Wert `hypercorn` setzt. Der Server
...
in `src\mitglied\repository\session.py` aktivieren.
...
```shell
    # Docker Hardened Image
    docker build --tag juergenzimmermann/mitglied:2026.4.1-hardened .
    # Debian Trixie slim
    docker build --tag juergenzimmermann/mitglied:2026.4.1-trixie --file Dockerfile.trixie .
    # Alpine
    docker build --tag juergenzimmermann/mitglied:2026.4.1-alpine --file Dockerfile.alpine .
```
...
```shell
    docker history juergenzimmermann/mitglied:2026.4.1-hardened
    docker history juergenzimmermann/mitglied:2026.4.1-trixie
    docker history juergenzimmermann/mitglied:2026.4.1-alpine
```
...
```shell
    docker inspect juergenzimmermann/mitglied:2026.4.1-hardened
    docker inspect juergenzimmermann/mitglied:2026.4.1-trixie
    docker inspect juergenzimmermann/mitglied:2026.4.1-alpine
```
...
```shell
    docker sbom juergenzimmermann/mitglied:2026.4.1-hardened
    docker sbom juergenzimmermann/mitglied:2026.4.1-trixie
    docker sbom juergenzimmermann/mitglied:2026.4.1-alpine
```

### Docker Compose

Mit _Docker Compose_ kann man die Anwendung mitglied z.B. zusammen mit PostgreSQL
starten. Dazu ist der _Service_ `mitglied` in der Datei `compose.yml` im Verzeichnis.
`extras\compose\mitglied` unter Verwendung des Images so konfiguriert, dass er erst dann
...
stattdessen _hypercorn_ für ASGI verwenden möchte, so setzt man in `extras\compose\mitglied\mitglied.toml`
im Block `mitglied.asgi` beim Schüssel `server` den Wert `hypercorn`.

```shell
    # Windows
    cd extras\compose\mitglied
    # macOS / Linux
    cd extras/compose/mitglied

    # Voraussetzung: Keycloak als Container ist separat gestartet
    # Shell fuer mitglied zzgl. DB-Server und Mailserver
    # mitglied mit Basis-Image Debian Trixie (Hardened Image ohne Shell, Alpine mit ash siehe Kommentar)
    docker compose up
    ...
    docker compose exec mitglied bash
        id
        env
        exit
...
```shell
    docker scout quickview juergenzimmermann/mitglied:2026.4.1-hardened
    docker scout quickview juergenzimmermann/mitglied:2026.4.1-trixie
    docker scout quickview juergenzimmermann/mitglied:2026.4.1-alpine
```
...
```shell
    docker scout cves juergenzimmermann/mitglied:2026.4.1-hardened
    docker scout cves --format only-packages juergenzimmermann/mitglied:2026.4.1-hardened
```
