...
DB `mitglied` mit dem DB-User `mitglied`anzulegen. Mit dem 2. Skript wird das
Schema `mitglied` mit dem DB-User `mitglied` als _Owner_ angelegt:
...
- Wie kann ich Mitglieder aus dem Postleitzahlengebiet "1" selektieren?
- Den Benutzernamen benötige ich nicht.
- Wie kann ich die Daten der 1. Abfrage als CSV exportieren?

**BEACHTE**:

- _Mitglieder_ als Plural der DB-Tabelle `mitglied`.
